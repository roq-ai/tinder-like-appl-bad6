...
export function MatchListPage(props: MatchListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getMatches({
        relations: ['user_match_user1_idTouser', 'user_match_user2_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_user1_idTouser',
          header: 'User Match User 1 Id Touser',
          accessorKey: 'user_match_user1_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_user1_idTouser?.id}`}
            >
              {record.user_match_user1_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_match_user2_idTouser',
          header: 'User Match User 2 Id Touser',
          accessorKey: 'user_match_user2_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_match_user2_idTouser?.id}`}
            >
              {record.user_match_user2_idTouser?.email}
            </Link>
          ),
        }
      : null,
